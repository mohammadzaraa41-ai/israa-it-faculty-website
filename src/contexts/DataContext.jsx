...
export const DataProvider = ({ children }) => {
  ...
  const addProject = async (project) => {
    const newProject = {
      ...
      notes_en: project.notes?.en || project.notes_en,
      files: project.files,
      images: project.images
    };

    let result = await supabase.from('project_bank').insert([newProject]).select();
    
    // Robust fallback if some columns (like name_en, notes_en, files, images) are missing
    if (result.error && (result.error.message.includes('column') || result.error.code === '42703')) {
      ...
      result = await supabase.from('project_bank').insert([minimalProject]).select();
    }

    const { data, error } = result;
    if (data) setProjectBank(prev => [...prev, {
      ...data[0],
      name: { ar: data[0].name_ar, en: data[0].name_en || data[0].name_ar },
      notes: { ar: data[0].notes_ar, en: data[0].notes_en || '' }
    }]);
    if (error) console.error("Project Bank Insert Error:", error);
  };
