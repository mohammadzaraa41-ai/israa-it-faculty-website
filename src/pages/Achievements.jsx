...
const Achievements = () => {
  ...
  const resetForm = () => {
    setFormData({ title: { ar: '', en: '' }, summary: { ar: '', en: '' }, report: { ar: '', en: '' }, year: '1', participants: { ar: '', en: '' }, images: [''], date: new Date().toISOString().split('T')[0] });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    let res;
    if (editingAch) {
      res = await editAchievement({ ...formData, id: editingAch.id });
    } else {
      res = await addAchievement(formData);
    }
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', marginBottom: '3rem' }}>
                  <div style={{ borderLeft: lang === 'ar' ? 'none' : '4px solid var(--accent-color)', borderRight: lang === 'ar' ? '4px solid var(--accent-color)' : 'none', padding: '1rem 1.5rem', background: 'rgba(255,255,255,0.03)' }}>
                    ...
                    <p style={{ margin: 0, color: 'var(--text-secondary)', lineHeight: '1.6' }}>
                      {selectedAch.participants?.[lang] || selectedAch.participants?.ar || selectedAch.participants?.en || (typeof selectedAch.participants === 'string' ? selectedAch.participants : '')}
                    </p>
                  </div>
                </div>
