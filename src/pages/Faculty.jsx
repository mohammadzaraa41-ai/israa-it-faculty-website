...
const Faculty = () => {
  ...
  const facultyMembers = adminFaculty || [];

  const resolveName = (name) => {
    if (!name) return '---';
    if (typeof name === 'object') return name[lang] || name.ar || name.en || '---';
    return name;
  };

  const getDeptName = (deptId) => {
    const dept = (departments || DB_SCHEMA.departments).find(d => d.id === deptId);
    if (!dept) return deptId || '---';
    const name = dept.name;
    if (typeof name === 'object') return name[lang] || name.ar || name.en || deptId;
    return dept.name_ar || dept.name || deptId;
  };

  const resolveDept = (member) => {
    if (member.department) return member.department;
    return getDeptName(member.departmentId);
  };

  const deans = facultyMembers.filter(m => m.role.includes('العميد'));
  const heads = facultyMembers.filter(m => m.role.includes('رئيس قسم'));
  const doctors = facultyMembers.filter(m => !m.role.includes('العميد') && !m.role.includes('رئيس قسم'));
          ...
          {deans.length > 0 && (
            <section style={{ marginBottom: '6rem' }}>
              <h2 style={{ textAlign: 'center', color: 'var(--primary-light)', marginBottom: '2.5rem', fontSize: '2rem' }}>
                {lang === 'ar' ? 'عميد الكلية' : 'Faculty Dean'}
              </h2>
