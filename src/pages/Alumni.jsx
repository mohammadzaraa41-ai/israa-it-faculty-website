...
// --- Sub-components (Moved Outside to prevent losing focus) ---
...
const ProjectSection = ({ isAdmin, lang, projectBank, addProject, deleteProject, editProject }) => {
        ...
        {isAdding && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="glass-panel" style={{ width: '100%', maxWidth: '800px', maxHeight: '90vh', overflowY: 'auto', padding: '2.5rem', position: 'relative' }}>
              <button style={{ position: 'absolute', top: '1.5rem', left: '1.5rem', color: '#ff4444', background: 'none', border: 'none', cursor: 'pointer' }} onClick={() => setIsAdding(false)}>✕</button>
              <h3 style={{ fontSize: '1.5rem', fontWeight: 'bold', marginBottom: '2rem', textAlign: 'center' }}>{editingProjectId ? (lang === 'ar' ? 'تعديل مشروع' : 'Edit Project') : (lang === 'ar' ? 'إضافة مشروع تخرج جديد' : 'Add New Graduation Project')}</h3>
              <form onSubmit={handleAddProjectClick} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                  <div>
                    ...
                    <input type="text" className="glass-panel" style={{ width: '100%', padding: '0.75rem', background: 'rgba(255,255,255,0.05)', color: 'white' }} value={newProj.name.ar} onChange={e => setNewProj({...newProj, name: {...newProj.name, ar: e.target.value}})} required />
                  </div>
                  <div>
                    <label style={{ fontSize: '0.85rem', display: 'block', marginBottom: '0.5rem' }}>{lang === 'ar' ? 'اسم المشروع (EN) *' : 'Project Name (EN) *'}</label>
                    <input type="text" className="glass-panel" style={{ width: '100%', padding: '0.75rem', background: 'rgba(255,255,255,0.05)', color: 'white' }} value={newProj.name.en} onChange={e => setNewProj({...newProj, name: {...newProj.name, en: e.target.value}})} required />
                  </div>
                </div>

                <div>
                  <label style={{ fontSize: '0.85rem', display: 'block', marginBottom: '0.5rem' }}>{lang === 'ar' ? 'اسم المشرف *' : 'Supervisor Name *'}</label>
                  <input type="text" className="glass-panel" style={{ width: '100%', padding: '0.75rem', background: 'rgba(255,255,255,0.05)', color: 'white' }} value={newProj.supervisor} onChange={e => setNewProj({...newProj, supervisor: e.target.value})} required />
                </div>
                ...
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '0.5rem' }}>
                    {newProj.students.map((s, i) => (
                      <input key={i} type="text" placeholder={`${lang === 'ar' ? 'طالب' : 'Student'} ${i+1}`} className="glass-panel" style={{ width: '100%', padding: '0.5rem', background: 'rgba(255,255,255,0.03)', fontSize: '0.8rem', color: 'white' }} value={s} onChange={e => {
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: '0.25rem' }}>
                      {[0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5].map(val => (
                        <button key={val} type="button" onClick={() => setNewProj({...newProj, rating: val})} style={{ padding: '0.25rem', color: newProj.rating >= val ? 'var(--accent-color)' : '#555', background: 'none', border: 'none', cursor: 'pointer' }}>
                  ...
                  <div>
                    ...
                    <input type="url" className="glass-panel" style={{ width: '100%', padding: '0.75rem', background: 'rgba(255,255,255,0.05)', color: 'white' }} value={newProj.link} onChange={e => setNewProj({...newProj, link: e.target.value})} />
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                  <div>
                    ...
                    <textarea className="glass-panel" style={{ width: '100%', padding: '0.75rem', background: 'rgba(255,255,255,0.05)', minHeight: '80px', color: 'white' }} value={newProj.description.ar} onChange={e => setNewProj({...newProj, description: {...newProj.description, ar: e.target.value}})} />
                  </div>
                  <div>
                    <label style={{ fontSize: '0.85rem', display: 'block', marginBottom: '0.5rem' }}>{lang === 'ar' ? 'وصف المشروع (EN)' : 'Project Description (EN)'}</label>
                    <textarea className="glass-panel" style={{ width: '100%', padding: '0.75rem', background: 'rgba(255,255,255,0.05)', minHeight: '80px', color: 'white' }} value={newProj.description.en} onChange={e => setNewProj({...newProj, description: {...newProj.description, en: e.target.value}})} />
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                  <div>
                    ...
                    <textarea className="glass-panel" style={{ width: '100%', padding: '0.75rem', background: 'rgba(255,255,255,0.05)', minHeight: '80px', color: 'white' }} value={newProj.notes.ar} onChange={e => setNewProj({...newProj, notes: {...newProj.notes, ar: e.target.value}})} />
                  </div>
                  <div>
                    <label style={{ fontSize: '0.85rem', display: 'block', marginBottom: '0.5rem' }}>{lang === 'ar' ? 'ملاحظات المشرف (EN)' : 'Supervisor Notes (EN)'}</label>
                    <textarea className="glass-panel" style={{ width: '100%', padding: '0.75rem', background: 'rgba(255,255,255,0.05)', minHeight: '80px', color: 'white' }} value={newProj.notes.en} onChange={e => setNewProj({...newProj, notes: {...newProj.notes, en: e.target.value}})} />
                  </div>
                </div>

                <button type="submit" className="btn-primary" disabled={isSubmitting} style={{ width: '100%', padding: '1rem', marginTop: '1rem', opacity: isSubmitting ? 0.7 : 1 }}>
                  {isSubmitting ? (lang === 'ar' ? 'جاري الحفظ...' : 'Saving...') : (lang === 'ar' ? 'حفظ المشروع ونشره' : 'Save & Publish Project')}
                </button>
              </form>
            </motion.div>
          </motion.div>
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {cvTemplates.map(cv => (
              <div key={cv.id} onClick={() => setSelectedCv(cv)} style={{ 
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <FileText size={16} color="var(--accent-color)" />
                  <span>{getLoc(cv, 'name', lang)}</span>
                </div>
                {isAdmin && (
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    <Edit2 size={16} color="var(--primary-light)" style={{ cursor: 'pointer' }} onClick={(e) => {
                      e.stopPropagation();
                      setModalData({ ...cv, nameAr: cv.name_ar, nameEn: cv.name_en });
                      setActiveModal('cv');
                    }} />
            ...
            {selectedCv && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ marginTop: '1rem', padding: '1rem', background: 'rgba(255,255,255,0.05)', borderRadius: '10px', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                  ...
                  <button className="btn-outline" style={{ padding: '0.5rem 1rem', fontSize: '0.8rem' }} onClick={() => handleDownload(selectedCv.url, getLoc(selectedCv, 'name', lang) + '.' + (selectedCv.file.split('.').pop()))}><Download size={14} /> {lang === 'ar' ? 'تحميل' : 'Download'}</button>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="glass-panel" style={{ padding: '1.5rem' }}>
          ...
          <AdminControl isAdmin={isAdmin} label={lang === 'ar' ? 'إضافة فيديو/أسئلة' : 'Add Video/Questions'} onAdd={handleAddInterview} />
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {interviewResources.map(res => (
              <div key={res.id} style={{ padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  {isAdmin && <Edit2 size={16} color="var(--primary-light)" style={{ cursor: 'pointer' }} onClick={() => { setModalData(res); setActiveModal('interview'); }} />}
        ...
        <div className="glass-panel" style={{ padding: '1.5rem' }}>
          ...
          <AdminControl isAdmin={isAdmin} label={lang === 'ar' ? 'إضافة نصيحة/فيديو' : 'Add Tip/Video'} onAdd={handleAddLinkedin} />
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
...
const Alumni = () => {
  ...
  const handleRetry = async () => {
    // Note: Use proper supabase reference or logic here if needed
    localStorage.removeItem(localPendingKey);
    setHasLocalPending(false);
  ...
  const handleViewFile = (url) => {
    if (!url || url === '#') {
      alert(lang === 'ar' ? 'معاينة غير متاحة' : 'Preview not available');
      return;
    }
    window.open(url, '_blank');
  };
  ...
  const handleDownload = async (url, fileName) => {
    if (!url || url === '#') {
      alert(lang === 'ar' ? 'ملف تجريبي' : 'Demo file');
      return;
    }
    try {
      const response = await fetch(url);
  ...
  return (
    <div style={{ padding: '1rem 2rem 4rem', maxWidth: '1200px', margin: '0 auto', minHeight: '100vh' }}>
        ...
        {activeModal && (
          <AdminModal 
            ...
            title={
              ...
              activeModal === 'interview' ? (lang === 'ar' ? 'بيانات المقابلة' : 'Interview Data') :
            ...
            onSave={async () => {
              ...
              try {
                if (activeModal === 'template') {
                  ...
                  if (modalData.id) {
                    await editCvTemplate(modalData.id, modalData.nameAr, lang);
                  } else {
                    await addCvTemplate({ 
                      name: { ar: modalData.nameAr, en: modalData.nameEn }, 
                  ...
                  if (modalData.id) {
                    await editInterviewResource(modalData.id, {
                  ...
                  if (modalData.id) {
                    await editLinkedinTip(modalData.id, {
                      title: { ar: modalData.titleAr, en: modalData.titleEn },
                      type: modalData.type,
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {(activeModal === 'template' || activeModal === 'cv') ? (
                <>
                  <input type="text" placeholder={lang === 'ar' ? 'الاسم بالعربي' : 'Name in Arabic'} className="glass-panel" style={{ width: '100%', padding: '0.8rem', background: 'rgba(255,255,255,0.05)', color: 'white' }} value={modalData.nameAr || ''} onChange={e => setModalData({...modalData, nameAr: e.target.value})} />
                  <input type="text" placeholder={lang === 'ar' ? 'الاسم بالإنجليزي' : 'Name in English'} className="glass-panel" style={{ width: '100%', padding: '0.8rem', background: 'rgba(255,255,255,0.05)', color: 'white' }} value={modalData.nameEn || ''} onChange={e => setModalData({...modalData, nameEn: e.target.value})} />
                </>
              ) : (
                <>
                  <input type="text" placeholder={lang === 'ar' ? 'العنوان بالعربي' : 'Title in Arabic'} className="glass-panel" style={{ width: '100%', padding: '0.8rem', background: 'rgba(255,255,255,0.05)', color: 'white' }} value={modalData.titleAr || (modalData.title?.ar || '')} onChange={e => setModalData({...modalData, titleAr: e.target.value})} />
                  <input type="text" placeholder={lang === 'ar' ? 'العنوان بالإنجليزي' : 'Title in English'} className="glass-panel" style={{ width: '100%', padding: '0.8rem', background: 'rgba(255,255,255,0.05)', color: 'white' }} value={modalData.titleEn || (modalData.title?.en || '')} onChange={e => setModalData({...modalData, titleEn: e.target.value})} />
                  <select className="glass-panel" style={{ width: '100%', padding: '0.8rem', background: 'rgba(255,255,255,0.05)', color: 'white' }} value={modalData.type || 'video'} onChange={e => setModalData({...modalData, type: e.target.value})}>
                  ...
                  {modalData.type === 'tip' ? (
                    <textarea placeholder={lang === 'ar' ? 'النصيحة...' : 'Tip content...'} className="glass-panel" style={{ width: '100%', padding: '0.8rem', background: 'rgba(255,255,255,0.05)', minHeight: '100px', color: 'white' }} value={modalData.content || ''} onChange={e => setModalData({...modalData, content: e.target.value})} />
                  ) : (
                    <input type="url" placeholder={lang === 'ar' ? 'الرابط URL' : 'Link URL'} className="glass-panel" style={{ width: '100%', padding: '0.8rem', background: 'rgba(255,255,255,0.05)', color: 'white' }} value={modalData.url || ''} onChange={e => setModalData({...modalData, url: e.target.value})} />
                  )}
                </>
              )}
            </div>
      ...
      <input type="file" ref={cvFileInputRef} style={{ display: 'none' }} onChange={handleCvFileChange} />
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'career' && (
            <CareerSection 
              ...
              deleteCvTemplate={deleteCvTemplate}
              deleteInterviewResource={deleteInterviewResource}
              deleteLinkedinTip={deleteLinkedinTip}
              handleViewFile={handleViewFile}
              handleDownload={handleDownload}
              handleAddCvTemplate={handleAddCvTemplate}
