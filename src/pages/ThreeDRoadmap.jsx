    ...
    <Float speed={3} rotationIntensity={0.5} floatIntensity={1.5}>
      <group position={position} onClick={onClick}>
        ...
        <Text 
          ...
          textAlign="center"
        >
          {typeof label === 'string' ? label : (label?.[lang] || label?.ar || label?.en || '')}
        </Text>
      </group>
    </Float>
  );
...
const RoadmapScene = ({ majorKey, lang }) => {
  ...
  return (
    <group ref={groupRef}>
      <Stars radius={100} depth={50} count={window.innerWidth < 768 ? 2000 : 7000} factor={4} saturation={0} fade speed={1.5} />
      ...
      {major.nodes.map((node, i) => (
        <CourseNode 
          ...
          label={node.name?.[lang] || node.name?.ar || node.name?.en || node.name}
          color={major.color}
          delay={i * 0.1}
      ...
      <header style={{ textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '0.5rem', marginTop: '0.75rem' }}>
          {Object.entries(majorRoadmaps).map(([key, data]) => (
            ...
            >
              {data.title?.[lang] || data.title?.ar || data.title?.en || data.title}
            </button>
          ))}
        </div>
      </header>
      ...
      >
        ...
        <div style={{
          ...
          <h2 style={{
            ...
            {majorRoadmaps[selectedMajor].title?.[lang] || majorRoadmaps[selectedMajor].title?.ar || majorRoadmaps[selectedMajor].title?.en || ''}
          </h2>
          {!isMobile && (
