      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' }}>
        {offeredCourses.map((c, i) => (
          <div 
            ...
            style={{ padding: '1.5rem', background: 'var(--surface-color)', borderRadius: '12px', border: '1px solid var(--surface-border)', position: 'relative' }}
          >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {studentTips.map((tip, i) => (
          <motion.div 
            ...
            style={{ display: 'flex', gap: '1rem', alignItems: 'center', background: 'rgba(241, 196, 15, 0.1)', padding: '1.5rem', borderRadius: '12px', borderLeft: '4px solid #f1c40f', borderBottom: '1px solid var(--surface-border)', position: 'relative' }}
          >
...
const CodingQuests = ({ lang }) => {
  const { user } = useAuth();
  const { quests, addQuest, deleteQuest, editQuest, reorderQuests, completeQuest, studentQuests } = useAdmin();
  const isAdmin = ['SUPER_ADMIN', 'DEAN', 'HOD', 'DOCTOR'].includes(user?.role);
  
  // XP Logic based on real database state
  const myCompletedQuests = useMemo(() => {
    if (!user || !Array.isArray(studentQuests)) return [];
    return studentQuests.filter(sq => sq.student_id === user.id);
  }, [user, studentQuests]);

  const xp = useMemo(() => {
    return myCompletedQuests.reduce((acc, sq) => {
      const quest = quests.find(q => q.id === sq.quest_id);
      return acc + (quest ? quest.xp : 0);
    }, 0);
  }, [myCompletedQuests, quests]);
  ...
  const handleComplete = async (questId) => {
    if (!user) return alert(lang === 'ar' ? 'يجب تسجيل الدخول أولاً' : 'Please login first');
    const res = await completeQuest(questId, user.id);
    if (!res.success) {
      alert(lang === 'ar' ? 'فشل في حفظ الإنجاز' : 'Failed to save achievement');
    }
  };
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {quests.map((quest, i) => {
          const isDone = myCompletedQuests.some(sq => sq.quest_id === quest.id);
          return (
            <div 
              key={quest.id} 
              ...
              style={{ 
                ...
                background: isDone ? 'rgba(46, 204, 113, 0.1)' : 'var(--surface-color)', 
                padding: '1.5rem', borderRadius: '12px', 
                border: `1px solid ${isDone ? '#2ecc71' : 'var(--surface-border)'}`,
                opacity: isDone && editingId !== quest.id ? 0.8 : 1,
                position: 'relative',
              ...
              {editingId === quest.id ? (
                ...
                <>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    {isDone ? <CheckCircle color="#2ecc71" size={24} /> : <Code color="var(--text-secondary)" size={24} />}
                    <span style={{ fontSize: '1.1rem', color: isDone ? '#2ecc71' : 'var(--text-primary)', textDecoration: isDone ? 'line-through' : 'none' }}>
                      {quest.title}
                    </span>
                  </div>
